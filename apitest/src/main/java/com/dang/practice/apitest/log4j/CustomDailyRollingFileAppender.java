...
public class CustomDailyRollingFileAppender extends FileAppender {
    ...
    int computeCheckPeriod() {  
        RollingCal RollingCal = new RollingCal(gmtTimeZone,  
                Locale.getDefault());
        ...
        if (datePattern != null) {  
            for (int i = TOP_OF_MINUTE; i <= TOP_OF_MONTH; i++) {  
                ...
                RollingCal.setType(i);  
                Date next = new Date(RollingCal.getNextCheckMillis(epoch));  
...
/** 
*  RollingCal is a helper class to DailyRollingFileAppender. 
*  Given a periodicity type and the current time, it computes the 
*  start of the next interval.  
* */  
class RollingCal extends GregorianCalendar {  
    ...
    RollingCal(TimeZone tz, Locale locale) {  
        super(tz, locale);  
    }  
