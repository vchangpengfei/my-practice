...
//    private  Cursor myCursor = null;
    ...
    //数据库文件名
    private  String fileName = "testdbfile";
    //数据库名称
    ...
    public  void openDatabase() {
        ...
        try{
            ...
            */
            ...
            EntityStore entityStore = new EntityStore(myDbEnvironment, "testtable", storeConfig);
            dataAccessor = new DataAccessor(entityStore);
    ...
    public  void openDatabasenew() {
        ...
        try{
            ...
            myDbEnvironment = new Environment(file, environmentConfig);
            entityStore = new EntityStore(myDbEnvironment, "testtable", storeConfig);
            dataAccessor = new DataAccessor(entityStore);
        }
        catch(DatabaseException e){
            System.out.println(e.getMessage());
    ...
    public void put(){
        ...
        for(int i=0;i<10;i++){
            Entity entity=new Entity(i,System.currentTimeMillis(),"hotelname"+i);
            dataAccessor.envStatsByHotelid.put(entity);
    ...
    public void get(long start,long end){
        EntityCursor<Entity> cursor = dataAccessor.envStatsBytime.entities(start, true, end, false);
        for (Entity stats : cursor) {
            System.out.println(stats);
        }
        cursor.close();
    }

    public void get(int hotelid){
        List<Entity> list = new ArrayList<Entity>();
        Entity entity = dataAccessor.envStatsByHotelid.get(hotelid);
       System.out.println(entity);
    }

    public void getMax(){
        List<Entity> list = new ArrayList<Entity>();
        EntityCursor<Integer> cursor = dataAccessor.envStatsByHotelid.keys();

        System.out.println(cursor.first());
        System.out.println(cursor.last());
//        for (long stats : cursor) {
//            System.out.println(stats);
//        }
        cursor.close();
    }
    ...
    public void gettimeMax(){
        EntityCursor<Long> cursor = dataAccessor.envStatsBytime.keys();
        System.out.println(cursor.first());
        System.out.println(cursor.last());
...
//        try{
//            test1.openDatabase();
//            test1.writeToDatabase("test1","val1",true);
//            test1.writeToDatabase("test1","test1",false);
//            test1.readFromDatabase("test1");
//        }catch (Throwable e){
//            e.printStackTrace();
//        }finally {
//            test1.closeDatabase();
//        }
        ...
        try{
            test2.openDatabasenew();
//            test2.put();
//            test2.get(3);
//            test2.get(Long.parseLong("1522325530053"),Long.parseLong("1522325530055"));
